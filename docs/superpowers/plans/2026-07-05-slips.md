...
### Task 12: CI structure — tests, quality gates, releases, dependencies

**Files:**
- Create: `.github/workflows/ci.yml`, `.github/workflows/release-please.yml`, `.github/workflows/auto-merge.yml`, `.github/dependabot.yml`

**Interfaces:**
- Produces: required status checks named `test (ubuntu-latest)`, `test (macos-latest)`, `test (windows-latest)`, `lint`, `typecheck` (referenced by Task 14 branch protection). Release pipeline: release-please on `main` → tagged GitHub release → npm publish with provenance.

- [ ] **Step 1: Write `.github/workflows/ci.yml`**

Cross-platform matrix matters here: slips does file locking and atomic renames, and agents run it on all three OSes. Lint/typecheck are platform-independent, so they run once.
...
jobs:
  test:
    ...
    steps:
      ...
      - run: bun test --reporter=junit --reporter-outfile=junit.xml --coverage --coverage-reporter=lcov
...
- [ ] **Step 2: Write `.github/workflows/release-please.yml`** — versioning, changelog, tag, GitHub release, then npm publish gated on an actual release being cut:
...
Publish prerequisites (do NOT block this task on them — file slips in Task 13 instead): the `NPM_TOKEN` secret must be added to the repo, and the npm package name `slips` must be checked for availability (`npm view slips` — if taken, pick a scoped name in `package.json` before the first release). Until the secret exists, the publish job simply fails on the first release and can be re-run after setup.

- [ ] **Step 3: Write `.github/dependabot.yml`** — keep actions and dev-dependencies fresh; PRs target `dev` per the branching model:
...
- [ ] **Step 4: Write `.github/workflows/auto-merge.yml`** — auto-merge dependabot minor/patch PRs once required checks pass (majors stay manual):
...
```bash
git add .github && git commit -m "ci: add cross-platform tests, quality gates, release automation, and dependabot"
```
...
```bash
bun src/cli.ts create --title "Add NPM_TOKEN secret and verify npm package name availability" --type task --priority 2 --label release
bun src/cli.ts create --title "Publish to npm" --type task --priority 3 --label release --blocked-by <id-of-npm-token-slip>
...
```bash
ml record architecture --type decision --description "JSONL single-file storage with merge=union + dedup-on-read; chosen over per-issue files for scan speed and a single fast read path for prime/ready"
...
Note: `pr-title` is not a required context (it runs solely on pull requests, so requiring it would wedge pushes that arrive outside PRs, e.g. release-please tag commits). `required_pull_request_reviews` is null deliberately: requiring reviews would deadlock a solo maintainer working with agents (you cannot approve your own PR). Consequence: direct pushes that pass the checks are technically possible — the PR-only flow is convention, not enforcement, in v1. Tighten with repository rulesets later if that proves insufficient.
...
Expected: repo public, default branch `dev`, protection lists the three `test (...)` matrix checks plus `lint` and `typecheck`.
